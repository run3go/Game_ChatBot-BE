...
class AnalysisGenerator:
    ...
    def analyze(self, question: str, history: list[dict] | None = None, candidates: list[str] | None = None, embedding_context: str = "") -> QuestionAnalysis:
        ...
        - MARKET: 거래소 아이템 시세 조회. 트리거: {{시세, 가격, 최저가, 전일가, 어제가, 실거래가, 최근가, 가성비, 비싼, 저렴한, 올랐어, 내렸어, 거래소}} 중 하나 + 아이템명(강화 재료·각인서·아바타·소모품·혼돈의 젬 등 레벨 없는 재료형 아이템). 닉네임 유무 무관.
          ...
          ⚠️ 각인서 + 가격·시세·비용·변동 키워드 → embedding_context에 각인(ENGRAVING)이 표시되어 있어도 반드시 MARKET. GLOBAL_ENGRAVING·ENGRAVING으로 분류 절대 금지.
            예) "예리한 둔기 유물 각인서 가격 변동" → embedding_context에 "예리한 둔기 [각인]"이 있어도 MARKET.
        ...
        - 장신구 옵션 약어(치피/치적/추피/적피/낙인/아공/아피/무공)와 등급 약어(상/중/하), 이 둘의 조합(치피상, 치적중, 추피하 등)은 게임 용어이며 절대 닉네임이 아님.
          ⚠️ 이전 대화가 AUCTION이고 현재 질문에 옵션 약어·등급 조합이 있으면 → AUCTION 유지, nicknames=[].
        ...
        - 현재 질문에 닉네임이 없을 때: 먼저 닉네임 없이 질문을 처리할 수 있는지 판단.
          ...
          ⚠️ 이전 대화 닉네임 상속은 조사·요청어 제외 시 단어 1~2개짜리 짧은 후속 질문에만 허용. 질문에 게임 트리거(스킬·각인·정보 등)나 닉네임이 아닌 미인식 단어가 포함된 경우 이전 닉네임을 절대 가져오지 마. nicknames=[].
