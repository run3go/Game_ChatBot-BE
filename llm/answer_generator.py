...
class AnswerGenerator:
    ...
    def answer_tft_api(self, question: str, data: dict, history: list[dict] | None = None):
        prompt = ChatPromptTemplate.from_template("""
            ...
            ## 덱별 성적
            comp_stats 필드가 있고 비어있지 않을 때만 이 섹션을 출력해. 없으면 섹션 전체 생략.
            - 게임 수 내림차순으로 각 덱을 불렛 한 줄씩 출력:
              **덱이름**: N게임 · 승률 win_rate% · 순방률 top4_rate%
            - "기타"는 메타 덱으로 분류되지 않은 게임. 존재하면 마지막에 표시.

            ## 분석
            전체 게임을 종합해서 아래 항목을 각각 불렛 한 줄로 작성해:
            - **특성 경향**: 자주 활성화된 특성 조합 패턴
            - **주력 유닛**: core_units 필드(아이템 3개 풀착용 & 2회 이상 등장한 유닛 목록)를 그대로 참조해 빈도 높은 순으로 나열해. core_units가 비어있으면 "풀착용 주력 유닛 없음"으로 표시해.
    ...
    def answer_tft(self, question: str, data, history: list[dict] | None = None):
        ...
        prompt = ChatPromptTemplate.from_template("""
            ...
            [보유 기물 기반 덱 추천 질문 (comp_name + match_score 컬럼이 있는 경우)]
            - 성능 비교표 출력 금지. 덱 추천 형식으로만 작성.
            1. 보유 기물을 언급하며 추천 덱을 소개하는 한 줄 도입.
            2. 각 덱을 번호로 나열 (match_score 내림차순, tier 우선).
               형식: `N. [티어] 덱이름 (보유 기물 M개 활용)`
               - 핵심 기물: champs_items에서 보유 기물과 겹치는 것 먼저, 그 다음 핵심 기물 나열
               - 평균 순위와 승률은 괄호 안에 간단히 표시
            3. 마지막에 최우선 추천 덱과 이유 한 줄.

            [기물 성능 질문 (star_level + games + avg_place + win_rate_pct 컬럼이 있는 경우)]
            - 성급(star_level) | 게임 수 | 평균 순위 | 승률(%) 표 하나만 출력.
            - avg_place 낮을수록, win_rate_pct 높을수록 좋음. 각각 더 좋은 값에 볼드체.
            - 표 아래 한 줄 결론: 어느 성급이 가장 성능이 좋은지 수치 근거와 함께 작성.

            [아이템 전반 성능 질문 (games + avg_place + win_rate_pct 컬럼만 있는 경우)]
            - 게임 수 | 평균 순위 | 승률(%) 표 하나만 출력.
            - 표 아래 한 줄 결론: 평균 순위·승률 수치를 근거로 쓸만한지 여부 작성.
            - 없는 컬럼(star_level, item_name 등) 절대 생성 금지.

            [기물별 아이템 성능 질문 (item_name + games + avg_place 컬럼이 있는 경우)]
            - 아이템 | 게임 수 | 평균 순위 세 컬럼 표 하나만 출력.
            - avg_place가 낮을수록 좋으므로 가장 낮은 값에 볼드체.
            - 승률·Top4율 등 데이터에 없는 컬럼 절대 생성 금지.

            [유닛 간 아이템/성능 비교 질문 (equipped_unit 컬럼이 있는 경우)]
            1. equipped_unit별로 그룹화해 평균 승률·평균 순위·Top4율을 집계한 비교 표를 먼저 출력. 표 헤더는 반드시 한국어로 (equipped_unit → "유닛", win_rate → "평균 승률(%)", avg_place → "평균 순위", top4_rate → "Top4율(%)"). 더 좋은 값에 볼드체.
            2. 각 유닛의 대표 아이템 조합(unit_items 기준 상위 1~2개 조합)을 표 아래 간략히 나열.
            3. 마지막에 결론 한 줄: 어느 유닛이 더 적합한지 수치 근거와 함께 명시.
